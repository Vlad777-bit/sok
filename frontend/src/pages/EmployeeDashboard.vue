  ...
  <div>
    ...
    <section class="block">
      ...
      <div v-for="a in apps" :key="a.id" class="item">
        <div class="row2">
          <div><b>#{{ a.id }}</b> (client {{ a.client_id }})</div>
          <div class="status">{{ a.status }}</div>
        </div>
        <div class="muted">{{ a.purpose }}</div>
        <div class="muted">Сумма: {{ a.requested_amount }} | срок: {{ a.term_months }} мес.</div>
        <div class="muted" v-if="a.comment">Комментарий: {{ a.comment }}</div>
      </div>
    </section>
...
async function loadAll() {
  error.value = "";
  try {
    apps.value = await api.listApplications(50, 0);
